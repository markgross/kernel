...
#ifdef WL_DBG_DBG
/* Debugging block used for p2p issue of improper "type" passed. */
	pid_name_buf = kmalloc(sizeof(current->comm), GFP_KERNEL);
	get_task_comm(pid_name_buf, current);

	pr_debug("%s: PID=%d, PID_NAME=%s\n", __func__, current->pid, pid_name_buf);
	kfree(pid_name_buf);
#endif
...
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0) && LINUX_VERSION_CODE <= KERNEL_VERSION(3, 17, 0))
