...
static int wifi_plat_dev_drv_probe(struct platform_device *pdev)
{
	...
	/* In latest BRCM* drivers for 3.10+ kernels this call is deprecated and a set power function
	 * is not explicitly not being defined. It has been found that taking down the mmc_host in this
	 * manner can be unreliable and produce states unable to recover from (can prevent wifi from
	 * coming back online when requested).
