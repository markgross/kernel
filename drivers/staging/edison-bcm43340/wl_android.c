...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	else if (strncasecmp(command, CMD_SETIBSSBEACONOUIDATA,
...
#endif
	else if (strncasecmp(command, CMD_RTCOEX, strlen(CMD_RTCOEX)) == 0) {
		bytes_written = wl_android_set_rt_coex(net, command, priv_cmd.total_len);
	}
	else if (strncasecmp(command, CMD_CONFIG_RTCOEX, strlen(CMD_CONFIG_RTCOEX)) == 0)
